<?php
	...
	$result = mysqli_query($conn, "SELECT level FROM user u INNER JOIN user_task ut ON u.id_user = ut.id_user WHERE ut.id_task = 2 AND u.username = '$username';");
	...
	if ($result->level >= 4) {
		...
		$result = mysqli_query($conn, "SELECT id_batch FROM batch WHERE active = '1' AND id_task = '2' AND golden_tasks = 0 ORDER BY used_times, RAND(".$randval.") LIMIT '$limit';");
		...
		for ($i = 0; $i < $count; $i++) {
			...
			$out = mysqli_query($conn, "SELECT id_image FROM batch_image WHERE id_batch = '$id_batch' ORDER BY id_validation, RAND(".$randval.");");
	...
	for ($i = 0; $i < $count; $i++) {
		$result = mysqli_query($conn, "SELECT i.name_cropped_image, c.id_cluster, ti.state FROM image i INNER JOIN image_cluster c INNER JOIN task_image ti ON i.id_image = c.id_image AND i.id_image = ti.id_image AND c.id_cluster = ti.id_validation WHERE id_image = ".$image_ids[$i]." AND id_task = 2");
